...
- **抓取 → 解密管线：端到端跑通过**（2026-09-17，Windows 11 + 微信 PC 4.1.13.65）。产物经 ffprobe 复验：`遇见浦发-浦发银行灯光秀-1080p.mp4` —— 5,580,633 字节 / 33.173 秒 / HEVC 1920×1080 + AAC / 995 帧，首 box 为 `ftyp`（size=28）；另有抽帧 `frame-live1.jpg` 人工确认。
  边界：该次运行发生在本工具成型过程中（用的是如今这批脚本的前身），其抓包日志未留存，无法反推当时确切执行的命令。
- **发行版入口（CLI / 桌面版）的完整闭环尚未跑过**：`setup → 播放 → watch → key → decrypt → cleanup` 全程没有一次真实执行记录；上面那条产物不是通过这些入口产出的。
- 各单元/回归与构建自检均已通过并可复现：安全闸门 22 断言、证书链自检、证书陷阱 8 断言、代理还原 21 断言、进程归属 16 断言、`EXE-SELFTEST-OK`（单文件与便携版）。
- 一次真实尝试曾在 `cleanup` 阶段中断，留下自建 PAC 与孤儿代理（已按备份还原并加了三重证据归属判定 + 兜底），详见 README「已验证」C 节。
